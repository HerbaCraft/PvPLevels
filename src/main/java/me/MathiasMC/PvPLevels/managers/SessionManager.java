	...
	private void task(Player killed, String attacker, Entity killer) {
		if (!killsessiontime.containsKey(killed.getUniqueId() + "=" + attacker)) {
			int id = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, () -> {
				if (killsessiontime.containsKey(killed.getUniqueId() + "=" + attacker)) {
					String[] split = killsessiontime.get(killed.getUniqueId() + "=" + attacker).split("=");
			...
			killsessiontime.put(killed.getUniqueId() + "=" + attacker, id + "=" + plugin.getFileUtils().config.getInt("kill-session.time"));
